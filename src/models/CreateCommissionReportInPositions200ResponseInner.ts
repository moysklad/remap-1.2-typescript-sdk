/* tslint:disable */
/* eslint-disable */
/**
 * МойСклад JSON API
 * API для манипуляции с сущностями и создания отчетов в онлайн-сервисе МойСклад.  ## Аутентификация  МойСклад поддерживает аутентификацию по протоколу Basic Auth и с использованием токена доступа: - Basic Auth: заголовок `Authorization` со значением пары `логин:пароль`, закодированным в Base64 - Bearer Token: заголовок `Authorization` со значением `Bearer <Access-Token>`  ## Ограничения  - Не более 45 запросов за 3 секундный период от аккаунта - Не более 5 параллельных запросов от одного пользователя   - Не более 20 параллельных запросов от аккаунта - Не более 20 Мб данных в одном запросе - Максимум 1000 элементов в массиве - Обязательное использование сжатия gzip 
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import type { CommissionReportInPosition } from './CommissionReportInPosition.js';
import {
    instanceOfCommissionReportInPosition,
    CommissionReportInPositionFromJSON,
    CommissionReportInPositionFromJSONTyped,
    CommissionReportInPositionToJSON,
} from './CommissionReportInPosition.js';
import type { Errors } from './Errors.js';
import {
    instanceOfErrors,
    ErrorsFromJSON,
    ErrorsFromJSONTyped,
    ErrorsToJSON,
} from './Errors.js';

/**
 * @type CreateCommissionReportInPositions200ResponseInner
 * 
 * @export
 */
export type CreateCommissionReportInPositions200ResponseInner = CommissionReportInPosition | Errors;

export function CreateCommissionReportInPositions200ResponseInnerFromJSON(json: any): CreateCommissionReportInPositions200ResponseInner {
    return CreateCommissionReportInPositions200ResponseInnerFromJSONTyped(json, false);
}

export function CreateCommissionReportInPositions200ResponseInnerFromJSONTyped(json: any, ignoreDiscriminator: boolean): CreateCommissionReportInPositions200ResponseInner {
    if (json == null) {
        return json;
    }
    if (typeof json !== 'object') {
        return json;
    }
    if (instanceOfCommissionReportInPosition(json)) {
        return CommissionReportInPositionFromJSONTyped(json, true);
    }
    if (instanceOfErrors(json)) {
        return ErrorsFromJSONTyped(json, true);
    }

    return {} as any;
}

export function CreateCommissionReportInPositions200ResponseInnerToJSON(json: any): any {
    return CreateCommissionReportInPositions200ResponseInnerToJSONTyped(json, false);
}

export function CreateCommissionReportInPositions200ResponseInnerToJSONTyped(value?: CreateCommissionReportInPositions200ResponseInner | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    if (typeof value !== 'object') {
        return value;
    }
    if (instanceOfCommissionReportInPosition(value)) {
        return CommissionReportInPositionToJSON(value as CommissionReportInPosition);
    }
    if (instanceOfErrors(value)) {
        return ErrorsToJSON(value as Errors);
    }

    return {};
}

