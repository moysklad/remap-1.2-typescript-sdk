/* tslint:disable */
/* eslint-disable */
/**
 * МойСклад JSON API
 * API для манипуляции с сущностями и создания отчетов в онлайн-сервисе МойСклад.  ## Аутентификация  МойСклад поддерживает аутентификацию по протоколу Basic Auth и с использованием токена доступа: - Basic Auth: заголовок `Authorization` со значением пары `логин:пароль`, закодированным в Base64 - Bearer Token: заголовок `Authorization` со значением `Bearer <Access-Token>`  ## Ограничения  - Не более 45 запросов за 3 секундный период от аккаунта - Не более 5 параллельных запросов от одного пользователя   - Не более 20 параллельных запросов от аккаунта - Не более 20 Мб данных в одном запросе - Максимум 1000 элементов в массиве - Обязательное использование сжатия gzip 
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * Объект, содержащий поля алкогольной продукции
 * @export
 * @interface ProductAlcoholic
 */
export interface ProductAlcoholic {
    /**
     * Содержит акцизную марку
     * @type {boolean}
     * @memberof ProductAlcoholic
     */
    excise?: boolean | null;
    /**
     * Код вида продукции
     * @type {number}
     * @memberof ProductAlcoholic
     */
    type?: number | null;
    /**
     * Крепость
     * @type {number}
     * @memberof ProductAlcoholic
     */
    strength?: number | null;
    /**
     * Объём тары
     * @type {number}
     * @memberof ProductAlcoholic
     */
    volume?: number | null;
}


/**
 * Check if a given object implements the ProductAlcoholic interface.
 */
export function instanceOfProductAlcoholic(value: object): value is ProductAlcoholic {
    return true;
}

export function ProductAlcoholicFromJSON(json: any): ProductAlcoholic {
    return ProductAlcoholicFromJSONTyped(json, false);
}

export function ProductAlcoholicFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProductAlcoholic {
    if (json == null) {
        return json;
    }
    return {
        'excise': json['excise'] == null ? undefined : json['excise'],
        'type': json['type'] == null ? undefined : json['type'],
        'strength': json['strength'] == null ? undefined : json['strength'],
        'volume': json['volume'] == null ? undefined : json['volume'],
    };
}

export function ProductAlcoholicToJSON(json: any): ProductAlcoholic {
    return ProductAlcoholicToJSONTyped(json, false);
}

export function ProductAlcoholicToJSONTyped(value?: ProductAlcoholic | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    return {
        'excise': value['excise'],
        'type': value['type'],
        'strength': value['strength'],
        'volume': value['volume'],
    };
}

